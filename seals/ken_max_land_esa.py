...
reference_dataset = os.path.join(available_land_inputs_path, 'base_year_lulc_esa.tif')
...
for param_index, parameters in parameters_dict.items():
    ...
    arable_array, current_and_arable_array = available_land_op(
        ...
        available_land_inputs['base_year_lulc_esa'],
...
current_array = np.where((base_lulc == 10) | (base_lulc == 11) | (base_lulc == 12) | 
                         (base_lulc == 20) | (base_lulc == 30) | (base_lulc == 40), 
                         available_land_inputs['ha_per_cell'], 
                         0
                         )
