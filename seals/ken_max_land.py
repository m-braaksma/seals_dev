...
def available_land_op(parameters, *input_arrays):
    ...
    arable_array = np.where(
        ...
        # (input_arrays[10] >= 10) & (input_arrays[10] <= 180), 1, 0
    ) * input_arrays[11]

    # current_and_arable_array = np.where(((input_arrays[10] == 10) | (input_arrays[10] == 11) | (input_arrays[10] == 12) | (input_arrays[10] == 20) | (input_arrays[10] == 30) | (input_arrays[10] == 40)) | (arable_array > 0), input_arrays[11], 0)
...
# reference_dataset = '/Users/mbraaksma/Files/seals/projects/run_ken_max_land/cropland_seals/lulc_esa_2017.tif'
...
# Output current crop values raster
output_file_path = os.path.join(cur_dir, f"current.tif")
output_csv_path = os.path.join(cur_dir, f"current.csv")
base_lulc = load_tif_as_array(reference_dataset) 
current_array = np.where((base_lulc == 2), available_land_inputs['ha_per_cell'], 0)
# current_array = np.where((base_lulc == 10) | (base_lulc == 11) | (base_lulc == 12) | (base_lulc == 20) | (base_lulc == 30) | (base_lulc == 40), available_land_inputs['ha_per_cell'], 0)
save_raster(output_file_path, current_array, reference_dataset)
stats = zonal_stats(kenya_vector_path, output_file_path, stats='sum')
save_zonal_stats_to_csv(stats, output_csv_path)
...
final_df_csv_simple = final_df_csv[['MAPREGH', 'ADM1_EN', 'ADM1_PCODE', 'pyramid_id', 'current_and_arable_def4', 'current']]
